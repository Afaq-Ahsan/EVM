...
//####################### * Cast vote * ##########################
   bool started;
   bool ended;
   function start(uint endtime)public onlyOwner{ //enter 86400 if you want pooling start till 24 hours
    started = true;
    end_time = block.timestamp + endtime; 
    }
    ...
    function Vote(uint256 _cnic,string memory _name,string memory vote_to,uint256 id)public{
        ...
        require( !vote_casted[msg.sender],"already voted");
    ...
    function Fetch_Results()public returns(uint,uint){
